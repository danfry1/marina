...
_Terminology is defined in [CONTEXT.md](./CONTEXT.md); decisions in [docs/adr/](./docs/adr/)._
...
| Decision | Choice |
|---|---|
| **Unit (`Target`)** | A row is a **Target** — a dev workload acted on by verbs (see [ADR 0001](./docs/adr/0001-target-as-the-unit.md)). Port-centric where a port exists; aggregates a process subtree; CPU/mem rolled up. Survives child-PID churn. |
...
See [ADR 0002](./docs/adr/0002-anchor-and-subtree-boundary.md). The climb from
...
The resolution engine is exposed headless for scripts and agents (see
[ADR 0003](./docs/adr/0003-cli-agent-surface.md)). The TUI is the no-arg default;
...
- **`phys_footprint` memory** via `libproc` (Activity-Monitor-matching), with RSS
  fallback.
- **CLI / agent surface** (ADR 0003): `ls`/`kill`/`restart`/`url` with `--json`
